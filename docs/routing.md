...
单个路由中可以包含多个通配符，只要相邻的通配符之间能被无歧义地划分。同一路由中的两个通配符必须满足以下条件之一：

- 由静态路径块或正则路径块进行分隔，这些路径块对路径文本有限制，因此可以约束前一个通配符的匹配范围
- 前一个通配符设置了 `capture` 上限，从而限制其可匹配的路径块数量

占位符路径块（`{name}`）**不能**作为分隔符，因为占位符会接受任意单个路径块的内容，这会使其前后两个通配符之间的边界变得模糊。
